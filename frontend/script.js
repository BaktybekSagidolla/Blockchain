...
const contractAddress = '0x4AF39B9722dBFf91B8C52c4E5d1b4F74537F4B84';
const contractABI = [
  {
    "inputs": [],
...
provider.send("eth_requestAccounts", []).then(() => {
    provider.listAccounts().then((accounts) => {
        ...
        contract.on("CandidateAdded", (id, name) => {
          alert(`Новый кандидат добавлен: ${name} с ID ${id}`);
      });
      contract.on("Voted", (voter, candidateId) => {
        alert(`Проголосовал: ${voter} за кандидата с ID ${candidateId}`);

    });

    // Отслеживание завершения выборов
    contract.on("ElectionEnded", (totalVotes) => {
      alert(`Выборы завершены. Всего голосов: ${totalVotes}`);
    });

    contract.on("ElectionStarted", (candidates, duration) => {
      alert(`Выборы начались! Кандидаты: ${candidates.join(", ")}. Продолжительность: ${duration} секунд.`);
  });
...
const startElection = async function() {
    ...
    const _votingDuraion = electionDuration.value;

    await contract.startElection(_candidates, _votingDuraion);
    refreshPage();
